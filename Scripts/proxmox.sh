...
create_template() {
            ...
            qm set "$1" --scsi0 "$PROXMOX_STORAGE_POOL:0,import-from=/root/workingdir/$3,discard=on"  > /dev/null 2>&1
            qm set "$1" --boot order=scsi0 --scsihw virtio-scsi-single
            qm set "$1" --agent enabled=1,fstrim_cloned_disks=1
            qm set "$1" --ide2 ${PROXMOX_STORAGE_POOL}:cloudinit
...
if [ "$Download_DEBIAN_11" == "Y" ]; then
    ...
    create_template $((nVMID + 1)) "Template-Debian-11" "debian-11-template.qcow2" "https://cloud.debian.org/images/cloud/bullseye/latest/debian-11-genericcloud-amd64.qcow2"
fi
...
if [ "$Download_DEBIAN_12" == "Y" ]; then
    ...
    create_template $((nVMID + 2)) "Template-Debian-12" "debian-12-template.qcow2" "https://cloud.debian.org/images/cloud/bookworm/latest/debian-12-genericcloud-amd64.qcow2"
fi
...
if [ "$Download_DEBIAN_13" == "Y" ]; then
    ...
    create_template $((nVMID + 3)) "Template-Debian-13" "debian-13-template.qcow2" "https://cloud.debian.org/images/cloud/trixie/daily/latest/debian-13-genericcloud-amd64-daily.qcow2"
fi
...
if [ "$Download_UBUNTU_22_04_LTS" == "Y" ]; then
    ...
    create_template $((nVMID + 11)) "Template-Ubuntu-22.04" "ubuntu-22.04-template.img" "https://cloud-images.ubuntu.com/releases/22.04/release/ubuntu-22.04-server-cloudimg-amd64.img"
 fi
...
if [ "$Download_UBUNTU_24_04" == "Y" ]; then
    ...
    create_template $((nVMID + 12)) "Template-Ubuntu-24.04" "ubuntu-24.04-template.img" "https://cloud-images.ubuntu.com/releases/24.04/release/ubuntu-24.04-server-cloudimg-amd64.img"
fi
...
if [ "$Download_FEDORA_40" == "Y" ]; then
    ...
    create_template $((nVMID + 22)) "Template-Fedora-40" "fedora-40-template.qcow2" "https://download.fedoraproject.org/pub/fedora/linux/releases/40/Cloud/x86_64/images/Fedora-Cloud-Base-Generic.x86_64-40-1.14.qcow2"
fi
...
if [ "$Download_ROCKY_LINUX_9" == "Y" ]; then
    ...
    create_template $((nVMID + 31)) "Template-Rocky-9" "rocky-9-template.qcow2" "http://dl.rockylinux.org/pub/rocky/9/images/x86_64/Rocky-9-GenericCloud.latest.x86_64.qcow2"
fi
